...
void loop() {
  ...
  for (int i = 0; i < BP32_MAX_GAMEPADS; i++) {
    ...
    int input_1 = -myGamepad->axisY(); // y axis is flipped for some reason, so unflip it
    int input_2 = -myGamepad->axisRY();
    if (input_1 > 10) {
      analogWrite(PIN_MOTOR_1A, map(input_1, 10, 512, 0, 255));
      digitalWrite(PIN_MOTOR_1B, 0);
    } else if (input_1 < -10) {
      digitalWrite(PIN_MOTOR_1A, 0);
      analogWrite(PIN_MOTOR_1B, map(input_1, -512, -10, 255, 0));    
    } else {
      analogWrite(PIN_MOTOR_1A, 0);
      analogWrite(PIN_MOTOR_1B, 0);
    }

    if (input_2 > 10) { //handle right motor driving
      analogWrite(PIN_MOTOR_2A, map(input_2, 10, 512, 0, 255));
      digitalWrite(PIN_MOTOR_2B, 0);
    } else if (input_2 < -10) {
      digitalWrite(PIN_MOTOR_2A, 0);
      analogWrite(PIN_MOTOR_2B, map(input_2, -512, -10, 255, 0));    
    } else {
